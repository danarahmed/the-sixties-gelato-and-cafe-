...
**P1-1, P1-3 and P1-4** (migration `0027` and the app):

- **P1-1** as recommended, and wider. The database itself records every change
  to products, what the till sells, categories, stock items and their units,
  suppliers, business settings and locations — added, changed or deleted, on a
  screen or in SQL — with only the values that changed, before and after, and
  the person; a change made with no one signed in says so. Every price set,
  changed or withdrawn is recorded with the price it replaced, and each price
  says who set it. Opening stock is the owner's alone, with a reason. Batch
  recipes are recorded with their ingredients before and after, and
  `new_recipe_version` can no longer be called. Each sale line keeps the name
  it was sold under. The **Audit trail** screen narrows by kind of change,
  person and dates, shows the values before and after, and downloads as CSV.
- **P1-3** as recommended. A delivery line is its price per unit times the
  quantity, with its cost a base unit shown as it is typed. A cost more than
  25% from the item's average (or, with none on hand, its last delivery) is
  refused, with both figures, until the person confirms it; the confirmation
  is audited. Each item's card shows its price history. A bill's amount is
  typed from the invoice, not copied from the receipt.
- **P1-4** except the merge tool. Items (names, type, reorder and par levels,
  in use) and suppliers (name, what they supply, phone, in use) are corrected
  on their screens, each change audited with its reason; pack units are
  added, and a unit in use keeps its size. Names are unique among those in
  use, ignoring case, spaces and punctuation ("Dairy Co" and "Dairy Co." are
  one supplier). An item is taken out of use only with no stock and nothing
  that needs it; a supplier, only when owed nothing; neither receives
  anything more. The live data has no duplicate names (checked on 25
...
**P1-8** (migration `0029` and the app), as recommended, with two
differences:

- The **alert rules** of §7 that the system can check: cash below zero; a
  drawer not counted (§7's "day not closed": since `0024` a drawer count, not
  a day, covers the cash); a stock count left open; running out; below the
  reorder level, items never stocked included; a delivery price confirmed far
  from the cost (0027's check, which already stops a mistyped one, is shown
  for a week); sold with no recipe; an ingredient with no cost yet, named once
  with the products that use it; a margin under the target, or below cost;
  waste above its usual week; one person's voids, refunds, discounts and
  cancelled bills; card money not banked; platform money not received;
  supplier bills due; possible price typos; possible duplicate payments. Each
  says what happened, why it matters, how urgent (🔴 now, 🟠 soon), what to do,
  and how sure it is. A rule that needs history is quiet with under a week of
  it; running out waits on the day a delivery comes. A late sale can no
  longer happen (`0024`), and use-by dates come with P2-7.
- Alerts are **kept**: one open alert per rule and subject, resolving itself
  once the condition clears; **answered** with a note, or **snoozed** until a
  day with a reason, both on the audit trail. One that turns from orange to
  red asks again, and nobody answers an alert about their own exceptions.
- The **thresholds** are settings the owner or general manager changes on
  Settings, each with its default; each vendor may say how many days a
  delivery takes, and running out uses the item's last supplier's.
- The **dashboard** opens on what needs someone — 🔴, then 🟠 (one rule's
  alerts folded into one row), 🟢 when nothing does, 🔵 what has been
  answered or snoozed — then **yesterday's brief**, its facts, calculations
  and what to do kept apart, and today's figures below.
- The differences: the brief is **on the dashboard, not sent at 07:00** — the
  café has no channel for messages yet (P1-11 asks about monitoring and its
  cost), and with the day ending at midnight yesterday is complete from 00:00.
  And the rules are checked each time the dashboard opens, not by a
  background job, so an alert is never older than the page.

**P1-9** (migration `0030` and the app), as recommended:

- **Card takings are settled** on Sales, a run of days at a time, each day
  once it is over (the till takes cards until midnight): the till's card
  takings against the terminal's report and what reached the bank. The bank's
  money goes to 1020; the terminal's total less what arrived — the card
  company's fee — to the new **6500 Card and bank fees**; and any difference
  between the till and the terminal — a sale rung as card and paid in cash, or
  the reverse — to 6300 Cash over / short, with a note saying why. Days are
  settled in order; the latest settlement can be cancelled with a reason, and
  its days wait again.
- **Paid by card**, an expense or a supplier payment now comes out of the
  bank (1020), so 1010 holds only the takings.
- **Every platform sale carries the platform's order number**, typed from its
  tablet: the database refuses a platform sale without one, or with a number
  already recorded for that platform. **Delivery Platforms** shows what each
  platform owes, order by order and for how long, and what 1100 holds that no
  order explains.
- **A statement is matched** by pasting its rows from the platform's report.
  Each line is matched to its sale, or said not to be (no such order, already
  paid, voided, on the statement twice); the orders it leaves out are listed;
  and the payout journal is proposed: Dr 1020 the payout, Dr 5100 the
  commission, Dr 5200 the fees and anything short, Cr 1100 the orders' value.
  A person who keeps the books posts it, with a note for any line that is not
  a clean match; those lines are kept with the statement, not posted. A
  posted statement can be cancelled with a reason.
- The alerts for **card money not banked** and **platform money not
  received** use these: card takings older than the threshold not yet
  settled; each platform's orders past its payout cycle, by number; and 1100
  that no order explains. A margin alert no longer rounds up: 69.97% reads
  "69.9% margin", not "70%".

On the live books, the 19,000 in 1010 is the card takings of the days since
trading began, waiting for their first settlement. The 5,250 in 1100 is
explained by no order — the two platform sales before `0030` have no number —
and is flagged until a journal explains it, or the test records are cleared.
...
| Products, prices, menu    |      ✅      | Dated prices per channel; categories, photos, favourites; recipe costed as it is typed; suggested price; every change audited, before and after                                                                                                              | Sizes (one variant per product), add-ons, recipe history on screen                                                                                |
...
| Inventory                 |      ✅      | Stock ledger; waste; manager corrections; stock board and stock cards; items corrected and taken out of use; pack units; price history                                                                                                                       | Reorder list                                                                                                                                      |
...
| Purchasing & suppliers    |      ✅      | Receive goods at a price per unit, checked against the cost now, with landed costs; bills clear GRNI (goods received not invoiced); payments; duplicate invoices refused; bill numbers; suppliers corrected                                                  | Purchase orders (table unused), supplier returns, credit notes, reversing a receipt                                                               |
...
| Reconciliation            |      🟡      | Four checks: stock vs 1200, bills vs 2000, unbilled receipts vs 2050, sales vs revenue; card takings settled against the terminal and the bank, and platform receivable explained by order (`0030`)                                                          | Cash (1000) and the bank (1020) against their statements                                                                                          |
...
| Dashboard                 |      ✅      | What needs someone first — the alerts, red, orange, answered (`0029`); yesterday's brief; then today: net sales, gross profit, orders, average order, stock value, low-stock count; recent sales; reconciliation badge; every figure opens what is behind it | Comparisons beyond the brief's week; sales by hour                                                                                                |
| Delivery platforms        |      ✅      | Platform sales post to 1100 at the platform price, each with the platform's order number; what each platform owes, by order; a statement matched order by order and its payout posted by a person (`0030`)                                                   | A statement read from its file or from the platform itself (it is pasted today)                                                                   |
| People & roles            |      ✅      | 9 roles, enforced by the database; invite, roles, deactivate                                                                                                                                                                                                 | Location-scoped roles; manager PIN on the till                                                                                                    |
| Settings                  |      🟡      | Shown read-only; the alert thresholds are edited on the screen (`0029`)                                                                                                                                                                                      | Editing the other business rules (currently only by SQL)                                                                                          |
...
| Alerts & notifications    |      🟡      | The alert rules of §7 on the dashboard, kept, answered and snoozed, resolving themselves (`0029`)                                                                                                                                                            | Notifications outside the app (email, WhatsApp); use-by alerts (P2-7)                                                                             |
...
|    7 | **Card and Talabat settlement matching**: proposes the payout journal                                                                                                                                                                                                                    |   2   | A person posts it                 | Medium-high                         | ✅ `0030`: the match proposes the journal; a person posts it |
...
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------- |
| How did we do yesterday, and what needs me?                        | ✅ Alerts and yesterday's brief on the dashboard (`0029`)                                                   | —                                                                                                | P1-8       |
...
| Where should the money be?                                         | ✅ Cash by account (`0024`); card takings waiting to be settled; what each platform owes, by order (`0030`) | —                                                                                                | P0-3, P1-9 |
...
clears.
**In place since `0029`** (see Status), every rule below but the late sale, which `0024`
made impossible, and the use-by rule, which waits for P2-7.
...
| Card                   | ✅ `0030`: card takings settled a day at a time against the terminal and the bank, the fee to 6500 and a difference to 6300 with a note; card payments out come from 1020.             | P1-9       |
| Platforms              | ✅ `0030`: every platform sale has the platform's order number, once; what is owed is shown by order; a statement is matched and a person posts the payout.                            | P1-9       |
