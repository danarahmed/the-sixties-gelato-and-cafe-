...
The till fills the screen: the menu on the left, and on the right the order
you are working on, laid out like a ticket. **☰** (top corner) opens the
app's menu. Along the top:
...
- **Tap a category** (Coffee, Gelato…) or **★ Favourites** in the list beside
  the products (along the top on a phone); each shows how many products it
  has. Or
...
1. **⚡ Quick sale**, then pick the channel at the top of the order:
   **Dine-in**, **Takeaway**, **Direct delivery** or **Talabat**. The channel
   sets the price and the packaging taken from stock, so always pick the right
   one.
...
4. **✅ Sale recorded** appears with the **customer's number** in large
   ...
   start again at 1 every day, and no two tills give the same one.
5. **🖨 Print receipt + barista ticket** prints two copies, cut apart: the
   customer's **receipt**, with every detail, and the **barista's ticket**,
   with the number in large figures and what to make, notes included, without
   prices. Hand the ticket to the bar. Printed again, **🖨 Print receipt** is
...
2. Tap the products, then **💾 Save**. The table now shows what it owes and how
   long it has been open. Tap it again to add more. A bill has its number from
   the moment it is opened.
3. **The bar gets its ticket when the order is saved**: on a till that prints
   by itself it comes out on **💾 Save**; otherwise press **☕ Barista
   ticket**. More added later comes out as **Added to the order**, with only
   what was added; anything taken off is listed under **Taken off: do not
   make**. Pressed when the bar already has everything, **☕ Barista ticket**
   prints the whole order again, marked **Copy**.
4. When they ask for the bill: **🖨 Print bill**. The table turns amber:
   **waiting for payment**.
5. When they pay: open the table, **💵 Cash** or **💳 Card**, **Confirm
   payment**. The bill closes and the sale is recorded, with the bill's
   number. Its receipt prints on its own: the bar made the order when it was
   saved.
...
Slips are laid out for an **80 mm receipt printer** and print on any printer.
Set the receipt printer as the computer's **default printer**. In the print
window, choose that printer, **Margins: None**, and untick **Headers and
footers**. The browser remembers these settings. In the printer's own settings
(Windows: **Printing preferences**), choose the roll paper and **cut after each
page**: the receipt and the barista's ticket then come out as two slips. A
printer that cuts only at the end prints them one under the other, with a
dashed line to tear along.

**🖨** at the top of the till sets how this till prints; each till remembers its
own:

- **Print by itself**: the customer's receipt after every payment, and the
  barista's ticket as each order is placed (a quick sale when it is paid, a
  table's order when it is saved). Off, nothing prints until you press a print
  button.
- **Print the barista's ticket** (on unless you turn it off): the second copy,
  for the bar.
- **Test print** prints a sample receipt and ticket, to set the printer up by.
