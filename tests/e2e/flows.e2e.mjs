...
console.log("▸ cashier rings up two sales");
{
  ...
  const receipt = await page.locator("text=Sale recorded").locator("..").locator("..").textContent();
  ...
  await page.locator(".product-tile", { hasText: "Golden espresso" }).click();
  check(await page.getByRole("button", { name: /platform/i }).isVisible(), "a Talabat order offers only platform-paid");
...
check(sql("select count(*) from sales_order where status = 'completed'") === "3", "three completed orders in the database");
check(sql("select count(distinct idempotency_key) from sales_order") === "3", "each with its own idempotency key");
check(sql("select count(*) from journal_entry where reference_type = 'sales_order' and status = 'published'") === "3",
...
console.log("▸ manager voids one sale and refunds another");
{
  ...
  ok("voided");
  await open(page, "/orders");
  await page.locator("tbody tr", { has: page.getByRole("button", { name: "Refund" }) }).first().getByRole("button", { name: "Refund" }).click();
...
check(sql("select string_agg(status::text, ',' order by status::text) from sales_order") === "completed,refunded,voided",
  "one sale voided, one refunded, one untouched");
check(sql("select count(*) from audit_log where action in ('sale.void','sale.refund')") === "2", "both on the audit trail");
...
console.log("▸ manager records the rent");
{
  ...
  await page.waitForFunction(() => {
    const s = [...document.querySelectorAll("select")].find((x) => [...x.options].some((o) => o.value === "6000"));
    return s && s.value === "6000";
  }, null, { timeout: 5000 }).then(() => ok("the account is proposed: 6000 Rent"), () => check(false, "no account proposed"));
...
console.log("▸ owner posts and reverses a manual journal");
{
  ...
  await page.getByPlaceholder("Being the reason this entry is made").fill("Generator fuel paid from the till");
  const blocked = await selects.nth(0).locator("option").allTextContents();
  check(!blocked.some((o) => /^(1200|2000|2050|3100) /.test(o)), "subledger accounts are not offered for manual journals");
  await page.getByRole("button", { name: "Save and publish" }).click();
  await page.getByText(/Journal \d+ published/).waitFor({ timeout: 10000 });
  ok("published");
  await open(page, "/journals?show=manual");
  await page.locator("tbody tr", { hasText: "Generator fuel" }).getByRole("button", { name: "Reverse" }).click();
  ...
  check(sql("select count(*) from journal_entry where reference_type = 'reversal' and description like 'Reversal: paid by%'") === "1",
...
console.log("▸ counter counts blind; manager approves");
{
  ...
  check((await page.locator("th", { hasText: "Expected" }).count()) === 0 && !/expected_base|"expected"/.test(html),
  ...
  await page.waitForFunction((k) => document.body.innerText.includes(`${k} of ${k} counted`), n, { timeout: 20000 });
...
}
{
  ...
  check(/Expected/.test(await page.locator("main").textContent()), "the reviewer sees expected against counted");
...
check(sql("select status from stock_count order by started_at desc limit 1") === "approved", "the count is approved");
...
check(sql("select count(*) from work_shift where closed_at is not null") === "1", "the day is closed once");
...
console.log("▸ owner reads the books");
{
  ...
  check(/Net revenue/.test(await page.locator("#pnl").textContent()), "the P&L renders from the ledger");
  const csv = await page.request.get(BASE + "/reports/export?report=trial_balance");
  check(csv.ok() && (csv.headers()["content-type"] || "").includes("text/csv"), "trial balance downloads as CSV");
  await open(page, "/accounting");
  check(/Earlier periods are locked|No draft journals/.test(await page.locator("main").textContent()), "the closing checklist is shown");
  await ctx.close();
}
{
  const { ctx, page } = await signIn(browser, "cashier");
...
// ------------------------------------------------------- the ledger ties
check(sql("select count(*) from journal_entry e where status = 'published' and (select sum(debit) - sum(credit) from journal_line where journal_entry_id = e.id) <> 0") === "0",
