...
export default async function PlatformsPage() {
  ...
  const [money, orders] = await Promise.all([getPlatformMoney(), getSalesOrders(500)]);
  const names = Object.fromEntries(money.platforms.map((p) => [p.code, p.name]));
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      ...
      <p className="muted" style={{ marginTop: 0, fontSize: ".9rem" }}>
        ...
        carries the order number from the platform&apos;s tablet, and the platform&apos;s statement
        is matched to the sales by it. Direct delivery is not a platform sale — it is taken as cash
        or card.
      </p>
          ...
          {money.orders.length === 0 ? (
            <div style={{ marginBlockStart: 12 }}>
              <EmptyState
                ...
                hint="A Talabat sale rung up on the till waits here, by its order number, until a statement pays it."
              />
            </div>
          ) : (
